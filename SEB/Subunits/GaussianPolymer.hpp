...
class GaussianPolymer : public SubUnit{
    ...
    virtual void Init(string name, string tag, SymbolInterface *GEX)
    {
        ...
        Expression Rg   = GLEX->getSymbol("Rg", n);    // For polymers we use the Radius of gyration to define the size.
        Expression x    = GLEX->getSymbol("x", n);     // The natural dimensionless variable is x = (q Rg)^2
        ...
        Expression Rg2 = pow(Rg, 2);   // All sizes below have unit (distance)^2 and are expressed in terms of Rg^2.
