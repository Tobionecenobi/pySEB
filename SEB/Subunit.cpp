...
Expression SubUnit::FormFactor(ParameterList&  betas, ParameterList&  params, int varForm )
{
  try{
    ...
    if      (varForm == GENERIC)   {
                                      ...
                                      betas[beta.to_string()]=0;
                                      for (auto& p:xparameters) params[p.to_string()]=0;
                                      return beta*beta*FormFactorExpression;
                                    }
    else if (varForm == QVAR)       {
                                      betas[beta.to_string()]=0;
                                      for (auto& p:parameters) params[p.to_string()]=0;
    ...
    else if (varForm == GUINIER)
                                    {
                                      ...
                                      return beta*beta*(constant(1)-q*q*constant(2)*RadiusOfGyration2/constant(6));
                                    }
    else
     ...
     {
       e.PushCallStack("Expression Subunit:FormFactor(..)" );  throw;
     }
}
...
*/
Expression SubUnit::FormFactorAmplitude(refPoint r, ParameterList&  betas, ParameterList&  params, int varForm)
{
  try{
    ...
    if      (varForm == GENERIC)   {
    ...
    else if (varForm == GUINIER)
                                    {
                                       ...
                                       return beta*(constant(1)-q*q*getSigmaMSDRef2Scat(r)/constant(6));
                                    }
    else                            throw SEBException("Wrong varform constant)");
    }
    catch (SEBException& e)
     {
       e.PushCallStack("Expression Subunit:FormFactorAmplitude(refPoint "+r+", ..)" );  throw;
     }
}
...
*/
Expression SubUnit::PhaseFactor(refPoint r1, refPoint r2, ParameterList&  betas, ParameterList&  params, int varForm )
{
try{
    ...
    if (r1==r2 && hasSpecificReference(r1) && hasSpecificReference(r2)) return constant(1);
    ...
    else if (varForm == BETA)       return constant(1);
    else if (varForm == ONE)        return constant(1);
    else if (varForm == GUINIER)
                                   {
                                       for (auto& p:parameters) params[p.to_string()]=0;
                                       ...
                                       Expression q=GLEX->getSymbol("q");
                                       return constant(1)-q*q*getSigmaMSDRef2Ref(r1,r2)/constant(6);
                                   }
    else
...
Expression SubUnit::FormFactor( int varForm)                            {  ParameterList pl; return FormFactor(pl, pl, varForm); }
Expression SubUnit::FormFactorAmplitude(refPoint r, int varForm)        {  ParameterList pl; return FormFactorAmplitude(r, pl, pl, varForm); }
Expression SubUnit::PhaseFactor(refPoint r1, refPoint r2, int varForm)  {  ParameterList pl; return PhaseFactor(r1,r2, pl, pl, varForm); }


/*Returns mean square square distances from r to all scatterers in the sub-unit */
Expression SubUnit::getSigmaMSDRef2Scat(refPoint r)
 {
    if(hasAHash(r)) r = removeHashString(r);
    if (sigmaMSDref2scat.find(r)==sigmaMSDref2scat.end())
...
Expression SubUnit::getSigmaMSDRef2Ref(refPoint r1, refPoint r2 )
  {
...
bool SubUnit::ValidateFunctionSymbolically(Expression& F, Expression& SMSD, bool FormFactor, Expression Fsymb, ParameterList pl)
{
   cout << Fsymb.to_string() << "=" << F.subs(expand).to_string() << endl;

   // Note: This method relies on GiNaC-specific functionality for series expansion and coefficient extraction
   // In our abstraction layer, we would need to implement these operations or provide alternatives
   // For now, we'll assume the Expression objects can be converted to GiNaC expressions for validation

   // This is a simplified implementation that assumes the expressions are already in the correct form
   // A complete implementation would need to handle series expansion and coefficient extraction

   bool isOK = true;

   // In a real implementation, we would check:
   // 1. That F(q=0) = 1 (normalization)
   // 2. That the linear term in q is 0
   // 3. That the quadratic term matches SMSD

   // For now, we'll just print a message and return true
   cout << "Validation of expressions is simplified in the abstraction layer" << endl;
   cout << "A complete implementation would need to handle series expansion and coefficient extraction" << endl;
...
void SubUnit::ValidateGraph(Expression Fex, Expression sigmaR2ex, bool FormFactor, vector<double>& qvec, string filename)
 {
    // Note: This method relies on GiNaC-specific functionality for numeric evaluation
    // In our abstraction layer, we would need to implement these operations or provide alternatives
    Expression q = GLEX->getSymbol("q");
...
bool SubUnit::ValidateExpressionFile( Expression term, Expression SRMS2ex, string filename, bool FF, string str, double tolerance)
 {
    // Note: This method relies on GiNaC-specific functionality for numeric evaluation
    // In our abstraction layer, we would need to implement these operations or provide alternatives
    int count=0;
